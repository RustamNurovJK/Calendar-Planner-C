...
void saveTasksToFile() {
    ...
    for (int i = 0; i < 366; i++) fwrite(tasks[i], sizeof(Task), taskCount[i], f);
    fclose(f);
}
...
void loadTasksFromFile() {
    ...
    for (int i = 0; i < 366; i++) fread(tasks[i], sizeof(Task), taskCount[i], f);
    fclose(f);
}
...
void loadBackgroundImage() {
    ...
    // If not found, try parent directory (when running from Debug folder)
    if (!g_hBackgroundBitmap) {
        sprintf(filePath, "..\\backgrounds\\%s.bmp", monthNames[currentMonth - 1]);
        g_hBackgroundBitmap = (HBITMAP)LoadImageA(NULL, filePath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    }
...
void drawTaskPanel(HDC hdc, int x, int y, int width, int height) {
    ...
    if (taskCount[dayIndex] == 0) {
        ...
        for (int i = 0; i < taskCount[dayIndex] && i < 15; i++) {
            ...
            char priorityChar[30];
            if (tasks[dayIndex][i].priority == 1) strcpy(priorityChar, "[LOW] ");
            else if (tasks[dayIndex][i].priority == 2) strcpy(priorityChar, "[MEDIUM] ");
            else strcpy(priorityChar, "[HIGH] ");

            if (tasks[dayIndex][i].hasTime) {
                sprintf(taskText, "%s%s [%02d:%02d]", priorityChar, tasks[dayIndex][i].title,
                    tasks[dayIndex][i].hour, tasks[dayIndex][i].minute);
            }
            else {
                sprintf(taskText, "%s%s", priorityChar, tasks[dayIndex][i].title);
            }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_TIMER:
        ...
        else if (wParam == TIMER_ID_REMINDER) { KillTimer(hwnd, TIMER_ID_REMINDER); checkReminders(); removeOverdueTasks(); }
        break;
    case WM_COMMAND:
