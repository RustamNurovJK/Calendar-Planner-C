...
// ОСНОВНАЯ ФУНКЦИЯ ПРОВЕРКИ ПРОСРОЧЕННОСТИ
int isOverdue(int day, int month, int year, int hasTime, int hour, int minute) {
    Date targetDate = { day, month, year };
    Date today = getCurrentDate();
    int dateCompare = compareDates(targetDate, today);

    // Если дата меньше текущей - просрочена
    if (dateCompare < 0) return 1;

    // Если дата равна текущей
    if (dateCompare == 0) {
        // Если время не задано - не просрочена (ещё целый день)
        if (!hasTime) return 0;

        // Если время задано - проверяем
        time_t now = time(NULL);
        struct tm* tm = localtime(&now);
        int currentHour = tm->tm_hour;
        int currentMinute = tm->tm_min;

        // Если текущее время больше времени задачи - просрочена
...
void removeOverdueTasks() {
    ...
    for (int dayIdx = 0; dayIdx < 366; dayIdx++) {
        ...
        Date taskDate = getDateFromDayIndex(dayIdx, currentDate.year);
        for (int i = taskCount[dayIdx] - 1; i >= 0; i--) {
            if (!tasks[dayIdx][i].completed) {
                if (isOverdue(taskDate.day, taskDate.month, taskDate.year,
                    tasks[dayIdx][i].hasTime,
...
void showOverdueNotification() {
    Date today = getCurrentDate();
    int overdueCount = 0;
    char overdueText[4000] = "";
    char tempText[400] = "";

    for (int dayIdx = 0; dayIdx < 366; dayIdx++) {
        if (taskCount[dayIdx] == 0) continue;
        Date taskDate = getDateFromDayIndex(dayIdx, today.year);
            ...
            if (!tasks[dayIdx][i].completed) {
                if (isOverdue(taskDate.day, taskDate.month, taskDate.year,
                    ...
                    if (tasks[dayIdx][i].hasTime) {
                        sprintf(timeStr, " at %02d:%02d", tasks[dayIdx][i].hour, tasks[dayIdx][i].minute);
                    }

                    char dateStr[20];
                    sprintf(dateStr, " (%02d.%02d.%04d)", taskDate.day, taskDate.month, taskDate.year);

                    if (tasks[dayIdx][i].description[0] != '\0' && strlen(tasks[dayIdx][i].description) > 0) {
                        sprintf(tempText, "  * %s %s%s%s\n    -> %s\n\n",
                            priorityStr, tasks[dayIdx][i].title, dateStr, timeStr,
                            tasks[dayIdx][i].description);
                    }
                    else {
                        sprintf(tempText, "  * %s %s%s%s\n\n",
                            priorityStr, tasks[dayIdx][i].title, dateStr, timeStr);
...
void drawTaskPanel(HDC hdc, int x, int y, int width, int height) {
    ...
    if (taskCount[dayIndex] == 0) {
        ...
        for (int i = 0; i < taskCount[dayIndex] && i < 15; i++) {
            ...
            sprintf(taskText, "%s%s%s", prefix, tasks[dayIndex][i].title, timeStr);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        // Показываем уведомление о просроченных задачах сразу
    ...
    case WM_TIMER:
        ...
        else if (wParam == TIMER_ID_REMINDER) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        if (pt.x >= taskPanelX && pt.x <= taskPanelX + taskPanelW && pt.y >= taskPanelY && pt.y <= taskPanelY + taskPanelH) {
            ...
            for (int i = 0; i < taskCount[dayIndex]; i++) {
                if (pt.y >= taskY && pt.y <= taskY + 20) { selectedTaskIndex = i; InvalidateRect(hwnd, NULL, TRUE); break; }
                taskY += 25;
            }
